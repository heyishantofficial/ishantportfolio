    ...
    <>
      <MacWindow title="Notes Workspace — Official Resume Document" icon={FileText} onClose={onClose} width="max-w-6xl" isDark={true}>
        <div className="flex flex-col h-[560px] max-h-[80vh] select-none overflow-hidden text-slate-100 p-2 sm:p-4">
          ...
          {/* Main Content Area — Full-width Crisp Resume Viewer */}
          <div className="flex-1 bg-slate-950/60 rounded-2xl border border-white/10 overflow-hidden flex flex-col relative shadow-inner">
            <div className="flex-1 overflow-y-auto p-2 sm:p-4 flex flex-col items-center justify-start scrollbar-thin">
              <div className="relative group max-w-full">
                ...
                <img
                  ...
                  className="max-w-full h-auto rounded-xl shadow-2xl border border-white/20 object-contain cursor-zoom-in transition-transform duration-300 group-hover:scale-[1.005]"
                  onClick={() => setIsZoomed(true)}
                />
    ...
    <MacWindow title="Photos" icon={ImageIcon} onClose={onClose} width="max-w-2xl">
      <div className="space-y-4">
        ...
        <div className="flex justify-between items-center pb-2 border-b border-slate-200/60 dark:border-slate-800">
          <div className="flex items-center gap-2">
            <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm">Photos</h3>
          ...
          <div
            ...
            className={`relative border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer select-none ${
              isDragging
                ...
                : 'border-slate-300/80 dark:border-slate-700/80 hover:border-blue-400 dark:hover:border-blue-500 bg-slate-50/60 dark:bg-slate-900/30'
            }`}
          >
            <input
              ref={fileInputRef}
        ...
        {photos.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[55vh] overflow-y-auto pr-1">
            {photos.map((photo) => (
              <div
                ...
                className="group relative aspect-square rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 shadow-xs border border-slate-200/50 dark:border-slate-800 cursor-pointer hover:shadow-md hover:scale-[1.02] transition-all"
              >
                <img
                  src={photo.url}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2">
                  <p className="text-[11px] font-semibold text-white truncate drop-shadow-sm">{photo.name}</p>
                  <p className="text-[9px] text-white/80">{photo.createdAt}</p>
                </div>
                {isAdmin && (
                  <button
          ...
          !isAdmin && (
            <div className="py-14 flex flex-col items-center justify-center text-center">
              <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800/80 flex items-center justify-center text-slate-400 mb-3 shadow-inner">
                <ImageIcon className="w-7 h-7 stroke-[1.5]" />
              </div>
              <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300">No Photos Yet</h4>
              <p className="text-xs text-slate-400 max-w-xs mt-1">
                The photo dump is currently empty. Photos added in admin mode will appear here.
              </p>
            </div>
          )
        )}
      ...
      {selectedPhoto && (
        ...
        >
          ...
          >
            <button
              onClick={() => setSelectedPhoto(null)}
              className="absolute -top-10 right-0 w-8 h-8 rounded-full bg-white/20 hover:bg-white/40 text-white flex items-center justify-center backdrop-blur-md transition-all cursor-pointer"
              title="Close Preview"
            ...
            />
            {selectedPhoto.name && (
              <p className="text-white text-xs font-medium mt-3 tracking-wide drop-shadow-md">
                {selectedPhoto.name} {selectedPhoto.createdAt ? `• ${selectedPhoto.createdAt}` : ''}
              </p>
            )}
          </div>
        </div>
