...
import {
  ...
  FileText, Sparkles, Mail, Link2, FileType2, Info
} from 'lucide-react';
    ...
    <>
      <div className="flex items-center gap-1 shrink-0">
        <button
          ...
          disabled={cursor === 0}
          aria-label="Back"
          className="w-6 h-6 rounded-md flex items-center justify-center text-slate-600 dark:text-slate-300 enabled:hover:bg-black/5 dark:enabled:hover:bg-white/10 disabled:opacity-30"
        >
      ...
      <h2 className="text-[13px] font-bold tracking-tight text-slate-900 dark:text-slate-50 truncate">
        {node?.name || 'Finder'}
      </h2>

      <div className="ml-auto flex items-center gap-2 shrink-0">
        <div className="hidden sm:flex items-center rounded-md border border-black/10 dark:border-white/15 overflow-hidden">
          <button
            onClick={() => setView('grid')}
            aria-label="Icon view"
            aria-pressed={view === 'grid'}
            className={`w-7 h-6 flex items-center justify-center ${view === 'grid' ? 'bg-black/10 dark:bg-white/20 text-slate-900 dark:text-white' : 'text-slate-500'}`}
          >
          ...
          <button
            ...
            className={`w-7 h-6 flex items-center justify-center ${view === 'list' ? 'bg-black/10 dark:bg-white/20 text-slate-900 dark:text-white' : 'text-slate-500'}`}
          >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-28 sm:w-40 pl-7 pr-2 py-1 rounded-md bg-black/5 dark:bg-white/10 border border-transparent focus:border-[var(--os-accent)] text-[11px] outline-none text-slate-800 dark:text-slate-100 placeholder:text-slate-400"
          />
        </div>
      </div>
    </>
    ...
    >
      <div className="h-full flex" onClick={() => setMenu(null)}>
        ...
        <nav
          ...
          className="hidden sm:flex w-44 shrink-0 flex-col gap-0.5 p-2 border-r border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/[0.03] overflow-y-auto"
        >
          ...
          {DESKTOP_ORDER.map((id) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-2 py-1.5 rounded-md text-[12px] font-medium flex items-center gap-2 transition-colors ${
                  isHere
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          {/* Breadcrumb / path bar */}
          <div className="shrink-0 h-7 px-3 flex items-center gap-1 border-t border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/[0.03] text-[10px] text-slate-500 dark:text-slate-400 overflow-x-auto">
            {path.map((p, i) => (
              <React.Fragment key={p.id}>
                {i > 0 && <ChevronRight className="w-3 h-3 shrink-0 opacity-50" />}
                <button
                  onClick={() => p.kind === 'folder' && navigate(p.id)}
                  className="hover:text-[var(--os-accent)] font-medium whitespace-nowrap"
                >
                  {p.name}
                </button>
              </React.Fragment>
            ))}
            <span className="ml-auto pl-3 whitespace-nowrap">{children.length} item{children.length === 1 ? '' : 's'}</span>
          </div>
        </div>
      </div>
      ...
      {menu && (
        ...
        >
          ...
          <button
            ...
            className="w-full text-left px-3 py-1.5 hover:bg-[var(--os-accent)] hover:text-white flex items-center gap-2"
          >
...
function GridItem({ node, isSelected, onSelect, onOpen, onContextMenu }) {
  return (
    <div
      ...
      className={`p-2 rounded-lg flex flex-col items-center text-center gap-1.5 cursor-pointer transition-colors ${
        isSelected ? 'bg-[var(--os-accent)] text-white' : 'hover:bg-black/5 dark:hover:bg-white/10'
      }`}
    >
      <NodeIcon node={node} size={54} />
      <span className={`text-[11px] font-semibold leading-tight line-clamp-2 ${isSelected ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>
        {node.name}
      </span>
      <span className={`text-[9px] ${isSelected ? 'text-white/80' : 'text-slate-500 dark:text-slate-400'}`}>
...
function ListItem({ node, isSelected, onSelect, onOpen, onContextMenu }) {
  ...
  return (
    <div
      ...
      onDoubleClick={onOpen}
      onContextMenu={onContextMenu}
      className={`px-2 py-1.5 flex items-center gap-3 cursor-pointer rounded-md ${
        isSelected ? 'bg-[var(--os-accent)] text-white' : 'hover:bg-black/5 dark:hover:bg-white/10'
      }`}
    >
      <NodeIcon node={node} size={22} />
      <div className="min-w-0 flex-1">
        <div className={`text-[12px] font-semibold truncate ${isSelected ? 'text-white' : 'text-slate-800 dark:text-slate-100'}`}>
          {node.name}
        </div>
        {node.description && (
      ...
      <div className={`hidden sm:flex items-center gap-1.5 text-[10px] shrink-0 ${isSelected ? 'text-white/80' : 'text-slate-400'}`}>
